...
"""
    radialCoordinates(colLength, nCells, r_inner, r_outer)
Calculate radial coordinates for each cell in a radial flow system.
Returns a vector of radial positions at cell interfaces.
"""
function radialCoordinates(colLength, nCells, r_inner, r_outer)
    # Verify inputs
    @assert r_outer > r_inner "Outer radius must be greater than inner radius"
    @assert nCells > 0 "Number of cells must be positive"
    
    # Calculate cell spacing in radial coordinates
    Δr = (r_outer - r_inner) / nCells
    
    # Generate radial coordinates at cell interfaces
    r_coords = Vector{Float64}(undef, nCells + 1)
    for i in 0:nCells
        r_coords[i+1] = r_inner + i * Δr
    end
...
"""
    radialJacobian(r_coords, nCells, nNodes)
Calculate the Jacobian for radial transformation at each node.
Returns a matrix of Jacobian values (size nNodes × nCells).
"""
function radialJacobian(r_coords, nCells, nNodes)
    # Initialize Jacobian matrix
    J = zeros(nNodes, nCells)
    
    # Calculate Δr for each cell
    Δr = diff(r_coords)
    
    # Jacobian is 2πr for radial coordinates (area scaling)
    for cell in 1:nCells
        r_mid = (r_coords[cell] + r_coords[cell+1]) / 2
        J[:, cell] .= 2π * r_mid  # Constant within each cell
    end
...
"""
    radialVolumeFactors(r_coords)
Calculate volume scaling factors for radial coordinates.
Returns a vector of factors for each cell.
"""
function radialVolumeFactors(r_coords)
    nCells = length(r_coords) - 1
    factors = Vector{Float64}(undef, nCells)
    
    for i in 1:nCells
        r1 = r_coords[i]
        r2 = r_coords[i+1]
        factors[i] = π * (r2^2 - r1^2)  # Area of annular ring
    end
...
"""
    radialFluxScaling(r)
Calculate the flux scaling factor for radial flow at radius r.
Accounts for changing cross-sectional area in radial flow.
"""
function radialFluxScaling(r)
    return 1 / (2π * r)  # Inverse of circumference
end

# Add radial version of mass matrix calculation
function MMatrixRadial(nodes, polyDeg, r_coords, cell_idx, alpha=0.0, beta=0.0)
    # Get standard mass matrix
    M = MMatrix(nodes, polyDeg, alpha, beta)
    
    # Scale by radial factors
    r1 = r_coords[cell_idx]
    r2 = r_coords[cell_idx+1]
    scaling = π * (r2^2 - r1^2)  # Area of annular ring
    
    return M * scaling
end

# Add radial version of derivative matrix
function derivativeMatrixRadial(polyDeg, nodes, r_coords, cell_idx)
    # Get standard derivative matrix
    D = derivativeMatrix(polyDeg, nodes)
    
    # Scale by radial transformation
    Δr = r_coords[cell_idx+1] - r_coords[cell_idx]
    return D * (2/Δr)  # Account for radial coordinate transformation
end
