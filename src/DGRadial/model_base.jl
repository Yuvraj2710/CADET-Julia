...
struct AxialGeometry <: GeometryType end
struct RadialGeometry <: GeometryType end
...
mutable struct ConvDispOp
    ...
    strideNode::Int64
    strideCell::Int64
	geometry::GeometryType  # New field for geometry type
    r_coords::Vector{Float64}  # New field for radial coordinates (empty for axial)
	...
	function ConvDispOp(polyDeg,nCells,colLength,geometry::GeometryType=AxialGeometry(),r_inner=0.0)
		...
		# Calculate radial coordinates if needed
        r_coords = Float64[]
        if isa(geometry, RadialGeometry)
            r_coords = DGElements.radialCoordinates(colLength, nCells, r_inner, r_inner + colLength)
        end
...
mutable struct LRM <: ModelBase
	...
	function LRM(; nComp, colLength, d_ax, eps_c, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exact_integration=1, cross_section_area=1., geometry=AxialGeometry(), r_inner=0.0)
		
		# Get necessary variables for convection dispersion DG 
		ConvDispOpInstance = ConvDispOp(polyDeg,nCells,colLength,geometry,r_inner)
...
mutable struct LRMP <: ModelBase
	...
	# Default variables go in the arguments in the LRM(..)
	function LRMP(; nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exact_integration=1, cross_section_area=1.0,geometry=AxialGeometry(), r_inner=0.0)
...
mutable struct GRM <: ModelBase
	...
	# Default variables go in the arguments in the LRM(..)
	function GRM(; nComp, colLength, d_ax, eps_c, eps_p, kf, Rp, Dp, Rc=0.0, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, polyDegPore=4, nCells=8, exact_integration=1, cross_section_area=1.0,geometry=AxialGeometry(), r_inner=0.0)
