...
"""
    DGjacobianRadialConvBlock!(convBlock, _nNodes, u, _polyDerM, _exactInt, _invMM, _invWeights, _deltaZ, r_coords, cell_idx)
Radial version of the convection Jacobian block calculation.
Accounts for radial geometry in convection terms.
"""
function DGjacobianRadialConvBlock!(convBlock, _nNodes, u, _polyDerM, _exactInt, _invMM, _invWeights, _deltaZ, r_coords, cell_idx)
    fill!(convBlock, 0.0)
    
    # Calculate radial scaling factor
    r = (r_coords[cell_idx] + r_coords[cell_idx+1])/2
    area_scale = 1/(2π*r)
    
    # Convection block with radial scaling
    @. @views convBlock[1:_nNodes, 2:_nNodes+1] -= _polyDerM * area_scale

    if typeof(_exactInt) == exact_integration
        @. @views convBlock[1:_nNodes, 1] += _invMM[1:_nNodes, 1] * area_scale
        @. @views convBlock[1:_nNodes, 2] -= _invMM[1:_nNodes, 1] * area_scale
    else
        convBlock[1, 1] += _invWeights[1] * area_scale
        convBlock[1, 2] -= _invWeights[1] * area_scale
    end
...
"""
    getRadialGBlock(cell_idx, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)
Radial version of the auxiliary block calculation.
Accounts for radial geometry in dispersion terms.
"""
function getRadialGBlock(cell_idx, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)
    gBlock = zeros(_nNodes, _nNodes + 2)
    @. @views gBlock[:, 2:_nNodes+1] = _polyDerM
    
    # Radial scaling factors
    r_avg = (r_coords[cell_idx] + r_coords[cell_idx+1])/2
    area_scale = 1/(2π*r_avg)
    
    if typeof(_exactInt) == exact_integration
        if cell_idx != 1 && cell_idx != _nCells
            @. @views gBlock[:, 1] -= 0.5 * _invMM[:, 1] * area_scale
            @. @views gBlock[:, 2] += 0.5 * _invMM[:, 1] * area_scale
            @. @views gBlock[:, _nNodes + 1] -= 0.5 * _invMM[1:_nNodes, _nNodes] * area_scale
            @. @views gBlock[:, _nNodes + 2] += 0.5 * _invMM[1:_nNodes, _nNodes] * area_scale
        elseif cell_idx == 1
            @. @views gBlock[:, _nNodes + 1] -= 0.5 * _invMM[1:_nNodes, _nNodes] * area_scale
            @. @views gBlock[:, _nNodes + 2] += 0.5 * _invMM[1:_nNodes, _nNodes] * area_scale
        elseif cell_idx == _nCells
            @. @views gBlock[:, 1] -= 0.5 * _invMM[:, 1] * area_scale
            @. @views gBlock[:, 2] += 0.5 * _invMM[:, 1] * area_scale
        end
        gBlock .*= 2 / _deltaZ
    else
        if cell_idx == 0 || cell_idx == _nCells + 1
            return zeros(_nNodes, _nNodes + 2)
        end

        gBlock[1, 1] -= 0.5 * _invWeights[1] * area_scale
        gBlock[1, 2] += 0.5 * _invWeights[1] * area_scale
        gBlock[_nNodes, _nNodes + 1] -= 0.5 * _invWeights[_nNodes] * area_scale
        gBlock[_nNodes, _nNodes + 2] += 0.5 * _invWeights[_nNodes] * area_scale
        gBlock *= 2 / _deltaZ

        if cell_idx == 1
            gBlock[1, 2] -= 0.5 * _invWeights[1] * 2 / _deltaZ * area_scale
            if cell_idx == _nCells
                gBlock[1, 1] += 0.5 * _invWeights[1] * 2 / _deltaZ * area_scale
                gBlock[_nNodes, _nNodes + 2] -= 0.5 * _invWeights[_nNodes] * 2 / _deltaZ * area_scale
                gBlock[_nNodes, _nNodes + 1] += 0.5 * _invWeights[_polyDeg + 1] * 2 / _deltaZ * area_scale
            end
        elseif cell_idx == _nCells
            gBlock[_nNodes, _nNodes + 1] += 0.5 * _invWeights[_polyDeg + 1] * 2 / _deltaZ * area_scale
        end
...
"""
    DGjacobianRadialDispBlock(cell_idx, _exactInt, _nNodes, _polyDerM, _invMM, _deltaZ, _invWeights, _polyDeg, _nCells, r_coords)
Radial version of dispersion Jacobian block calculation.
Accounts for radial geometry in dispersion terms.
"""
function DGjacobianRadialDispBlock(cell_idx, _exactInt, _nNodes, _polyDerM, _invMM, _deltaZ, _invWeights, _polyDeg, _nCells, r_coords)
    ...
    if typeof(_exactInt) == exact_integration
        ...
        B = getBMatrix(_nNodes)
        gBlock = getRadialGBlock(cell_idx, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)
        gStarDC = auxBlockGstar(cell_idx, getRadialGBlock(cell_idx-1, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords), gBlock, getRadialGBlock(cell_idx+1, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords), _nNodes, _nCells)

        dispBlock[1:_nNodes, _nNodes + 1 : _nNodes+2+_nNodes] += _polyDerM * gBlock - _invMM * B * gBlock
        dispBlock += _invMM * B * gStarDC
        dispBlock .*= (2 / _deltaZ) * area_scale
        return dispBlock
    else
        dispBlock = zeros(_nNodes, 3 * _nNodes)
        GBlockLeft = getRadialGBlock(cell_idx - 1, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)
        GBlock = getRadialGBlock(cell_idx, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)
        GBlockRight = getRadialGBlock(cell_idx + 1, _nNodes, _polyDerM, _nCells, _invMM, _deltaZ, _invWeights, _polyDeg, _exactInt, r_coords)

        dispBlock[1:_nNodes, _nNodes:_nNodes+_nNodes+2-1] = _polyDerM * GBlock * area_scale

        if cell_idx > 1
            dispBlock[1, _nNodes] += -_invWeights[1] * (-0.5 * GBlock[1, 1] + 0.5 * GBlockLeft[_nNodes, _nNodes + 1]) * area_scale
            dispBlock[1, _nNodes + 1] += -_invWeights[1] * (-0.5 * GBlock[1, 2] + 0.5 * GBlockLeft[_nNodes, _nNodes+2]) * area_scale
            dispBlock[1, _nNodes + 2 : _nNodes+ 2 + _nNodes - 1] += -_invWeights[1] * (-0.5 * GBlock[1, 3:_nNodes+3-1]) * area_scale
            dispBlock[1, 1:_nNodes - 1] += -_invWeights[1] * (0.5 * GBlockLeft[_nNodes, 2:2+_nNodes-1-1]) * area_scale
        elseif cell_idx == 1
            dispBlock[1, _nNodes:_nNodes+_nNodes+2-1] += -_invWeights[1] * (-GBlock[1, 1:_nNodes+2]) * area_scale
        end
        
        if cell_idx < _nCells
            dispBlock[_nNodes, _nNodes : _nNodes + _nNodes-1] += _invWeights[_nNodes] * (-0.5 * GBlock[_nNodes, 1 : _nNodes]) * area_scale
            dispBlock[_nNodes, 2 * _nNodes] += _invWeights[_nNodes] * (-0.5 * GBlock[_nNodes, _nNodes + 1] + 0.5 * GBlockRight[1, 1]) * area_scale
            dispBlock[_nNodes, 2 * _nNodes + 1] += _invWeights[_nNodes] * (-0.5 * GBlock[_nNodes, _nNodes+2] + 0.5 * GBlockRight[1, 2]) * area_scale
            dispBlock[_nNodes, 2 * _nNodes + 2 : 2 * _nNodes + 2 + _nNodes - 1 - 1] += _invWeights[_nNodes] * (0.5 * GBlockRight[1, 3:3 + _nNodes - 1 - 1]) * area_scale
        elseif cell_idx == _nCells
            dispBlock[_nNodes, _nNodes:_nNodes+_nNodes+2-1] += _invWeights[_nNodes] * (-GBlock[_nNodes, 1:_nNodes+2]) * area_scale
        end
...
"""
    addRadialLiquidJacBlock1!(jacobian, block, _nCells, _nNodes, u, _nComp, compstride, r_coords)
Radial version of liquid Jacobian block addition.
Accounts for radial geometry in the full Jacobian assembly.
"""
function addRadialLiquidJacBlock1!(jacobian, block, _nCells, _nNodes, u, _nComp, compstride, r_coords)
    # Boundary for forward flow with radial scaling
    r = (r_coords[1] + r_coords[2])/2
    area_scale = 1/(2π*r)
    @. @views jacobian[1 + compstride:_nNodes + compstride, 1 + compstride: _nNodes + compstride] += u * block[1:end, 2:end] * area_scale

    # Iterating over remaining cells with radial scaling
    for i = 2:_nCells
        r = (r_coords[i] + r_coords[i+1])/2
        area_scale = 1/(2π*r)
        @. @views jacobian[1 + (i-1)*_nNodes + compstride: (i-1)*_nNodes + _nNodes + compstride, (i-1)*_nNodes + compstride : (i-1)*_nNodes + _nNodes + compstride] += u * block * area_scale
    end
    nothing
end
