...
module ConvDispOperatorDG
    ...
    """
    residualImplRadial!(Dh, y, idx, _strideNode, _strideCell, _nPoints, _nNodes, _nCells,
                      _deltaZ, _polyDeg, _invWeights, _polyDerM, _invMM, u, d_ax, cIn,
                      c_star, h_star, Dc, _h, mul1, _exactInt, r_coords)
Radial version of the convection-dispersion operator residual implementation.
Accounts for radial geometry in flux calculations and volume terms.
"""
function residualImplRadial!(Dh, y, idx, _strideNode, _strideCell, _nPoints, _nNodes, _nCells, _deltaZ, _polyDeg, _invWeights, _polyDerM, _invMM, u, d_ax, cIn, c_star, h_star, Dc, _h, mul1, _exactInt, r_coords)
    ...
    volumeIntegralRadial!(y, idx, Dc, _nCells, _nNodes, _polyDerM, mul1, r_coords)
    
    # Interface flux calculation
    interfaceFluxRadial!(c_star, y, idx, _strideNode, _strideCell, _nCells, r_coords)
    
    # Surface integral with radial scaling
    surfaceIntegralRadial!(Dc, y, idx, _strideNode, _strideCell, 1, _nNodes, c_star, _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    
    # Compute flux term with radial scaling
    @inbounds for i in 1:_nPoints
        cell_idx = div(i-1, _nNodes) + 1
        r = r_coords[cell_idx] + (r_coords[cell_idx+1] - r_coords[cell_idx]) * 
            (nodes[mod(i-1, _nNodes)+1] + 1)/2
        flux_scale = 1/(2π*r)
        _h[i] = 2.0 / _deltaZ * (u * flux_scale * y[idx[1] + i - 1] + 
                d_ax * (2.0 / _deltaZ)^2 * Dc[i])
    end
    
    # Volume integral of flux term
    volumeIntegralRadial!(_h, Dh, _nCells, _nNodes, _polyDerM, mul1, r_coords)
    
    # Interface flux for main equation
    interfaceFluxRadial!(h_star, y, idx, d_ax, Dc, u, _nCells, _nPoints, _deltaZ, cIn, _strideNode, _strideCell, 1, _strideCell, r_coords)
    
    # Final surface integral
    surfaceIntegralRadial!(Dh, _h, 1, _nNodes, _strideNode, _strideCell, h_star, _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    
    nothing
end

"""
    volumeIntegralRadial!(state, stateDer, _nCells, _nNodes, _polyDerM, mul1, r_coords)
Radial version of volume integral calculation.
Accounts for varying cell volumes in radial coordinates.
"""
function volumeIntegralRadial!(state, stateDer, _nCells, _nNodes, _polyDerM, mul1, r_coords)
    @inbounds for Cell in 1:_nCells
        Δr = r_coords[Cell+1] - r_coords[Cell]
        r_avg = (r_coords[Cell] + r_coords[Cell+1])/2
        scaling = 2π * r_avg * Δr / 2  # Volume scaling factor
        
        mul!(mul1, _polyDerM, @view(state[1 + (Cell-1)*_nNodes : Cell*_nNodes]))
        @. @views stateDer[1 + (Cell-1)*_nNodes : Cell*_nNodes] -= mul1 * scaling
    end
    nothing
end

"""
    interfaceFluxRadial!(_surfaceFlux, C, idx, d_ax, _g, u, _nCells, _nPoints, _deltaZ,
                       cIn, _strideNode, _strideCell, _strideNode_g, _strideCell_g, r_coords)
Radial version of interface flux calculation.
Accounts for changing cross-sectional area in radial flow.
"""
function interfaceFluxRadial!(_surfaceFlux, C, idx, d_ax, _g, u, _nCells, _nPoints, _deltaZ, cIn, _strideNode, _strideCell, _strideNode_g, _strideCell_g, r_coords)
    # Inner interfaces
    @inbounds for Cell in 2:_nCells
        r = r_coords[Cell]
        area_scale = 1/(2π*r)
        _surfaceFlux[Cell] = area_scale * (u * C[idx[1] + (Cell-1)*_strideCell - _strideNode] - 0.5 * (-2.0/_deltaZ) * d_ax * (_g[1 + (Cell-1)*_strideCell_g - _strideNode_g] + _g[1 + (Cell-1)*_strideCell_g]))
    end
    
    # Boundary fluxes with area scaling
    r_in = r_coords[1]
    r_out = r_coords[end]
...
Radial version of surface integral calculation.
Accounts for varying interface areas in radial coordinates.
"""
function surfaceIntegralRadial!(stateDer, state, strideNode_state, strideCell_state, strideNode_stateDer, strideCell_stateDer, _surfaceFlux, _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    if typeof(_exactInt) == exact_integration
        ...
        @inbounds for Cell in 1:_nCells
            ...
            stateDer[1 + (Cell-1)*strideCell_stateDer] -=
                _invWeights[1] * scale_in * (state[1 + (Cell-1)*strideCell_stateDer] - _surfaceFlux[Cell])
            
            stateDer[1 + (Cell-1)*strideCell_stateDer + _polyDeg*strideNode_stateDer] +=
                _invWeights[end] * scale_out * (state[1 + (Cell-1)*strideCell_stateDer + _polyDeg*strideNode_stateDer] - _surfaceFlux[Cell+1])
        end
    end
    nothing
end
