...
"""
    residualImplRadial(y, idx, _strideNode, _strideCell, _nPoints, _nNodes, _nCells,
                     _deltaZ, _polyDeg, _invWeights, _polyDerM, _invMM, u, d_ax, cIn,
                     _exactInt, r_coords)
Allocating version of radial convection-dispersion operator.
Returns the residual vector for radial flow systems.
"""
    function residualImplRadial(y, idx, _strideNode, _strideCell, _nPoints, _nNodes, _nCells, _deltaZ, _polyDeg, _invWeights, _polyDerM, _invMM, u, d_ax, cIn,               _exactInt, r_coords)
        # Initialize output vectors
        Dc = zeros(eltype(y), _nPoints)
        Dh = zeros(eltype(y), _nPoints)
        _h = zeros(eltype(y), _nPoints)
        c_star = zeros(eltype(y), _nCells+1)
        h_star = zeros(eltype(y), _nCells+1)
        
    #    Calculate radial scaling factors
        radial_factors = [1/(2π * (r_coords[i] + r_coords[i+1])/2) for i in 1:_nCells]
    
    # Volume integral with radial scaling
        Dc = volumeIntegralRadial(y, idx, _nCells, _nNodes, _polyDerM, r_coords)
    
    # Interface flux calculation
        c_star = interfaceFluxAuxiliaryRadial(y, idx, _strideNode, _strideCell, _nCells, r_coords)
    
    # Surface integral with radial scaling
        Dc = surfaceIntegralRadial(Dc, y, idx, _strideNode, _strideCell, 1, _nNodes, c_star, _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    
    # Compute flux term with radial scaling
        nodes, _ = DGElements.lglnodes(_polyDeg)
        @inbounds for i in 1:_nPoints
            cell_idx = div(i-1, _nNodes) + 1
            r = r_coords[cell_idx] + (r_coords[cell_idx+1] - r_coords[cell_idx]) * (nodes[mod(i-1, _nNodes)+1] + 1)/2
            flux_scale = 1/(2π*r)
            _h[i] = 2.0 / _deltaZ * (u * flux_scale * y[idx[1] + i - 1] + d_ax * (2.0 / _deltaZ)^2 * Dc[i])
        end
    
    # Volume integral of flux term
        Dh = volumeIntegralRadial(_h, _nCells, _nNodes, _polyDerM, r_coords)
    
    # Interface flux for main equation
        h_star = interfaceFluxRadial(y, idx, d_ax, Dc, u, _nCells, _nPoints, _deltaZ,
                                cIn, _strideNode, _strideCell, 1, _strideCell, r_coords)
    
    # Final surface integral
        Dh = surfaceIntegralRadial(Dh, _h, 1, _nNodes, _strideNode, _strideCell, h_star,
                              _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    
        return Dh
    end

"""
    volumeIntegralRadial(state, idx, _nCells, _nNodes, _polyDerM, r_coords)
Allocating version of radial volume integral calculation.
"""
function volumeIntegralRadial(state, idx, _nCells, _nNodes, _polyDerM, r_coords)
    stateDer = zeros(eltype(state), length(state))
    
    @inbounds for Cell in 1:_nCells
        Δr = r_coords[Cell+1] - r_coords[Cell]
        r_avg = (r_coords[Cell] + r_coords[Cell+1])/2
        scaling = 2π * r_avg * Δr / 2  # Volume scaling factor
        
        @views stateDer[1 + (Cell-1)*_nNodes : Cell*_nNodes] = 
            -_polyDerM * state[idx[1] + (Cell-1)*_nNodes : idx[1]-1 + Cell*_nNodes] * scaling
    end
...
"""
    interfaceFluxAuxiliaryRadial(C, idx, _strideNode, _strideCell, _nCells, r_coords)
Allocating version of radial auxiliary interface flux calculation.
"""
function interfaceFluxAuxiliaryRadial(C, idx, _strideNode, _strideCell, _nCells, r_coords)
    _surfaceFlux = zeros(eltype(C), _nCells+1)
    
    # Inner interfaces
    @inbounds for Cell in 2:_nCells
        r = r_coords[Cell]
        _surfaceFlux[Cell] = 0.5 * ((C[idx[1] + (Cell-1)*_strideCell - _strideNode] + C[idx[1] + (Cell-1)*_strideCell]) / (2π*r))
    end
    
    # Boundary interfaces
    _surfaceFlux[1] = 0.5 * ((C[idx[1]] + C[idx[1]]) / (2π*r_coords[1]))
    _surfaceFlux[_nCells+1] = 0.5 * ((C[idx[1] + _nCells*_strideCell - _strideNode] + C[idx[1] + _nCells*_strideCell - _strideNode]) / (2π*r_coords[end]))
    
    return _surfaceFlux
end

"""
    interfaceFluxRadial(C, idx, d_ax, _g, u, _nCells, _nPoints, _deltaZ, cIn,
                       _strideNode, _strideCell, _strideNode_g, _strideCell_g, r_coords)
Allocating version of radial interface flux calculation.
"""
function interfaceFluxRadial(C, idx, d_ax, _g, u, _nCells, _nPoints, _deltaZ, cIn, _strideNode, _strideCell, _strideNode_g, _strideCell_g, r_coords)
    _surfaceFlux = zeros(eltype(C), _nCells+1)
    ...
    @inbounds for Cell in 2:_nCells
        r = r_coords[Cell]
        area_scale = 1/(2π*r)
        _surfaceFlux[Cell] = area_scale * (u * C[idx[1] + (Cell-1)*_strideCell - _strideNode] -
                            0.5 * (-2.0/_deltaZ) * d_ax *
                            (_g[1 + (Cell-1)*_strideCell_g - _strideNode_g] +
                             _g[1 + (Cell-1)*_strideCell_g]))
    end
    
    # Boundary fluxes
    r_in = r_coords[1]
    r_out = r_coords[end]
    _surfaceFlux[1] = u * cIn / (2π*r_in)
    _surfaceFlux[_nCells+1] = (u * C[idx[1] + _nCells*_strideCell - _strideNode] - 0.5 * (-2.0/_deltaZ) * d_ax * (_g[1 + _nCells*_strideCell_g - _strideNode_g] + (-_g[_nPoints]))) / (2π*r_out)
    
    # Apply inverse mapping Jacobian
    @. _surfaceFlux *= 2.0 / _deltaZ
    return _surfaceFlux
end

"""
    surfaceIntegralRadial(stateDer, state, strideNode_state, strideCell_state,
                        strideNode_stateDer, strideCell_stateDer, _surfaceFlux,
                        _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
Allocating version of radial surface integral calculation.
"""
function surfaceIntegralRadial(stateDer, state, strideNode_state, strideCell_state, strideNode_stateDer, strideCell_stateDer, _surfaceFlux, _nCells, _nNodes, _invMM, _polyDeg, _invWeights, _exactInt, r_coords)
    if typeof(_exactInt) == exact_integration
        @inbounds for Cell in 1:_nCells
            ...
            @inbounds for Node in 1:_nNodes
                stateDer[1 + (Cell-1)*strideCell_stateDer + (Node-1)*strideNode_stateDer] -= (_invMM[Node, 1] * scale_in * (state[1 + (Cell-1)*strideCell_state] - _surfaceFlux[Cell]) - (_invMM[Node, end] * scale_out * (state[1 + (Cell-1)*strideCell_state + _polyDeg*strideNode_state] - _surfaceFlux[Cell+1])))
            end
        end
    else
        # Collocated version
        @inbounds for Cell in 1:_nCells
            ...
            stateDer[1 + (Cell-1)*strideCell_stateDer] -= _invWeights[1] * scale_in * (state[1 + (Cell-1)*strideCell_stateDer] - _surfaceFlux[Cell])
            
            stateDer[1 + (Cell-1)*strideCell_stateDer + _polyDeg*strideNode_stateDer] += _invWeights[end] * scale_out * (state[1 + (Cell-1)*strideCell_stateDer + _polyDeg*strideNode_stateDer] - _surfaceFlux[Cell+1])
        end
    end
    return stateDer
end
